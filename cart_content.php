...
<?php if ($cart_rs->num_rows != 0) { ?>
    <form action="checkout.php" method="post" id="cartForm1" name="cartForm1">
        ...
        <button type="button" id="btn03" name="btn03" class="btn btn-success" onclick="btn_confirmLink('確定要清空購物車？','shopcart_del.php?mode=2')">清空購物車</button>
                ...
                <tbody>
                    <?php while ($cart_data = mysqli_fetch_array($cart_rs)) { ?>
                        <tr>
                            ...
                            <td><button type="button" id="btn[]" name="btn[]" class="btn btn-danger" onclick="btn_confirmLink('確定要刪除本資料？','shopcart_del.php?mode=1&cartid=<?php echo $cart_data['cartid']; ?>')">取消</button></td>
                        </tr>
